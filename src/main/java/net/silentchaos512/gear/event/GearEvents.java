...
public final class GearEvents {
    ...
    public static void onAttackEntity(LivingAttackEvent event) {
        ...
        final float newDamage = TraitHelper.activateTraits(weapon, baseDamage, (trait, level, value) -> trait
                .onAttackEntity(new TraitActionContext(player, level, weapon), attacked, value));
            ...
            if (GearHelper.isGear(stack) && TraitHelper.hasTrait(stack, Const.Traits.FLAMMABLE)) {
                ...
                if (GearHelper.isBroken(stack)) {
                    event.getEntity().sendSystemMessage(
                            TextUtil.translate("trait", "flammable.itemDestroyed", stack.getHoverName()));
    ...
    private static float getReducedMagicDamageScale(float magicArmor) {
        // Scale linearly up to 60% for magic armor < 20. Above 20, scale half as fast
        // (40 = 90%)
        if (magicArmor > 20)
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        if (tool.getItem() instanceof ICoreItem) {
            ...
            if (tool.isCorrectToolForDrops(state)) {
                ...
                // use getNewSpeed() instead of getOriginalSpeed() to support other mods that
                // are changing the break speed with this event.
    ...
    public static void onBlockBreak(BlockEvent.BreakEvent event) {
                ...
                if (tool.getDamageValue() < tool.getMaxDamage() - logCount) {
                    ...
                    for (BlockPos logPos : connectedLogs) {
                        if (!logPos.equals(event.getPos())) { // Skip the original log
                            ...
                            event.getPlayer().level().playSound(null, logPos, SoundEvents.WOOD_BREAK,
        ...
        if (TraitHelper.hasTrait(tool, Const.Traits.JABBERWOCKY) && event.getState().is(Tags.Blocks.ORES_DIAMOND)
                && EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SILK_TOUCH, tool) == 0) {
            Entity entity = JABBERWOCKY_MOBS.get(SilentGear.RANDOM.nextInt(JABBERWOCKY_MOBS.size()))
                    .apply(event.getPlayer().getCommandSenderWorld());
    ...
    public static void onLivingDeath(LivingDeathEvent event) {
        ...
        if (killer instanceof Player && !killer.level().isClientSide) {
            ...
            if (TraitHelper.hasTraitEitherHand(player, Const.Traits.CONFETTI)) {
                for (int i = 0; i < 3; ++i) {
                    FireworkRocketEntity rocket = new FireworkRocketEntity(player.level(), event.getEntity().getX(),
                            event.getEntity().getEyeY(), event.getEntity().getZ(), createRandomFirework());
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (!event.player.level().isClientSide && event.phase == TickEvent.Phase.START) {
            ...
            // Turtle trait
            // TODO: May want to add player conditions to wielder effect traits, for more
            // control and possibilities for pack devs.
            if (!event.player.isEyeInFluid(FluidTags.WATER)
                    && TraitHelper.hasTrait(event.player.getItemBySlot(EquipmentSlot.HEAD), Const.Traits.TURTLE)) {
            ...
            if (event.player.getY() < -64 && TraitHelper.hasTraitArmor(event.player, Const.Traits.VOID_WARD)) {
                // A small boost to get the player out of the void, then levitation and slow
                // falling
    ...
    private static void tickMagnetic(Player player, int magneticLevel) {
        ...
        AABB aabb = new AABB(player.getX() - range, player.getY() - range, player.getZ() - range,
                player.getX() + range + 1, player.getY() + range + 1, player.getZ() + range + 1);
        for (ItemEntity entity : player.level().getEntitiesOfClass(ItemEntity.class, aabb,
                e -> e.distanceToSqr(player) < range * range)) {
            if (canMagneticPullItem(entity)) {
                ...
                if (entity.getY() < target.y) {
                    double xzDistanceSq = (entity.getX() - target.x) * (entity.getX() - target.x)
                            + (entity.getZ() - target.z) * (entity.getZ() - target.z);
    ...
    public static void onLivingFall(LivingFallEvent event) {
        ...
        if (event.getDistance() > 3 && event.getEntity() instanceof Player player) {
            ...
            // FIXME: bounce is very unpredictable. Usually it does not work at all. The few
            // times
            ...
            if (bounce > 0 && event.getDistance() > 3) {
                if (!event.getEntity().isSuppressingBounce()) {
                    ...
                    event.getEntity().level().playSound(null, event.getEntity().blockPosition(),
                            SoundEvents.SLIME_BLOCK_FALL, SoundSource.PLAYERS, 1f, 1f);
                    event.setCanceled(true);
                }
            }
        }
    ...
    public static void onPlayerTickBouncing(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END && !event.player.isFallFlying()
                && BOUNCE_TICKS.containsKey(event.player)) {
            // event.player.moveForced(event.player.getPosX(), event.player.getPosY() + 0.1,
            // event.player.getPosZ());
    ...
    private static void bounceEntity(Entity entity) {
        ...
        if (vector3d.y < 0) {
            // entity.moveForced(entity.getPosX(), entity.getPosY() + 0.1,
            // entity.getPosZ());
