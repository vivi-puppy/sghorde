...
public interface ITrait {
    ...
    /**
     * Add tooltip information for this trait. Normally, this consists of just the
     * trait's
     * translated name and level, but may include a description under certain
     * conditions. If the
     * trait is hidden ({@link #isHidden()}), nothing is shown unless advanced
     * tooltips are
     * enabled.
     *
     ...
     * @param tooltip    The tooltip list
     * @param flag       The tooltip flag
     * @param affixFirst A function which can be used to make additional changes to
     *                   the first line
     *                   (display name)
     */
    default void addInformation(int level, List<Component> tooltip, TooltipFlag flag,
            Function<Component, Component> affixFirst) {
        if (!showInTooltip(flag))
            return;

        // Display name
        Component displayName = TextUtil.withColor(this.getDisplayName(level),
                isHidden() ? ChatFormatting.DARK_GRAY : ChatFormatting.GRAY);
    ...
    void onGetAttributeModifiers(TraitActionContext context, Multimap<Attribute, AttributeModifier> modifiers,
            String slot);

    @Deprecated
    default void onGetAttributeModifiers(TraitActionContext context, Multimap<Attribute, AttributeModifier> modifiers,
            EquipmentSlot slot) {
    ...
    /**
     * Called when the player left-clicks with the item in their hand without
     * targeting a block or
     * an entity.
     *
    ...
    void onBlockMined(TraitActionContext context, BlockState state, BlockPos pos);
    ...
    /**
     * Gets the IDs of the traits that this trait will cancel with. This should only
     * be used to
     * provide information to players. It should <em>not</em> be used to actually
     * check if traits
     * will cancel.
     *
    ...
    /**
     * Used by the trait command's markdown dump subcommand
     * (TraitsCommand#runDumpMd). Note that the
     * trait is responsible for adding indentation, bullets, and newlines if
     * desired. A two-space
