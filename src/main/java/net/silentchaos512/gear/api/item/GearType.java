...
/**
 * Used for classifying gear for certain purposes, such as rendering. For
 * example, any armor type
 * can be matched individually (helmet, chestplate, etc.), or all together with
 * "armor".
 * <p>
 * New gear types can be added with {@link #getOrCreate(String, GearType)}. It
 * is recommended to
 * store this in a static final field in your item class, but the location
 * doesn't matter.
 */
public final class GearType {
    ...
    public static final GearType AXE = getOrCreate("axe", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.DEFAULT_AXE_ACTIONS));
    public static final GearType PICKAXE = getOrCreate("pickaxe", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.DEFAULT_PICKAXE_ACTIONS));
    public static final GearType SHOVEL = getOrCreate("shovel", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.DEFAULT_SHOVEL_ACTIONS));
    public static final GearType EXCAVATOR = getOrCreate("excavator", SHOVEL,
            b -> b.toolActions(ToolActions.SHOVEL_DIG));
    public static final GearType HAMMER = getOrCreate("hammer", PICKAXE, b -> b.toolActions(ToolActions.PICKAXE_DIG));
    public static final GearType SAW = getOrCreate("saw", AXE, b -> b.toolActions(ToolActions.AXE_DIG));
    public static final GearType HOE = getOrCreate("hoe", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.DEFAULT_HOE_ACTIONS));
    public static final GearType MATTOCK = getOrCreate("mattock", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.AXE_DIG, ToolActions.HOE_DIG, ToolActions.HOE_TILL, ToolActions.SHOVEL_DIG));
    public static final GearType PAXEL = getOrCreate("paxel", HARVEST_TOOL, b -> b.toolActions(
            ToolActions.AXE_DIG,
    ...
    public static final GearType SHEARS = getOrCreate("shears", HARVEST_TOOL,
            b -> b.toolActions(ToolActions.DEFAULT_SHEARS_ACTIONS));
    public static final GearType SICKLE = getOrCreate("sickle", HARVEST_TOOL, b -> b.toolActions(ToolActions.HOE_DIG));
    ...
    public static final GearType DAGGER = getOrCreate("dagger", MELEE_WEAPON,
            b -> b.toolActions(ToolActions.DEFAULT_SWORD_ACTIONS));
    ...
    public static final GearType BATTLEAXE = getOrCreate("battleaxe", MELEE_WEAPON,
            b -> b.toolActions(ToolActions.DEFAULT_AXE_ACTIONS));
    // Ranged weapons (bows)
    ...
    public static final GearType SHIELD = getOrCreate("shield", TOOL,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(337f / 15f));
    // Armor
    public static final GearType ARMOR = getOrCreate("armor", ALL,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY));
    public static final GearType BOOTS = getOrCreate("boots", ARMOR,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(13));
    public static final GearType CHESTPLATE = getOrCreate("chestplate", ARMOR,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(16));
    public static final GearType ELYTRA = getOrCreate("elytra", ARMOR,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(25));
    public static final GearType HELMET = getOrCreate("helmet", ARMOR,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(11));
    public static final GearType LEGGINGS = getOrCreate("leggings", ARMOR,
            b -> b.durabilityStat(() -> ItemStats.ARMOR_DURABILITY).armorDurabilityMultiplier(15));
    // Projectiles
    ...
    /**
     * Gets or creates a new gear type without a parent. This should NOT be used in
     * most cases. If
     * the gear type already exists, the existing instance is not modified in any
     * way.
     *
     * @param name The gear type name. Must be unique and contain only lowercase
     *             letters and
     *             underscores.
     * @return The newly created gear type, or the existing instance if it already
     *         exists
    ...
    private GearType(String name, @Nullable GearType parent, int animationFrames, Supplier<ItemStat> durabilityStat,
            float armorDurabilityMultiplier, Set<ToolAction> toolActions) {
        this.name = name;
        this.parent = parent;
    ...
    /**
     * Gets the parent gear type, if there is one. The parent type may also have a
     * parent.
     *
    ...
    /**
     * Check if this type's name matches the given string, or if its parent type
     * does. The type
     * "all" will match anything.
     *
     * @param type The string representation of the type
     * @return True if this type's name is equal to type, or if its parent matches
     *         (recursive)
     */
     ...
     *
     * @param type       The string representation of the type
     * @param includeAll Whether or not to consider the "all" type. This should be
     *                   excluded if
     *                   trying to match more specific types.
     * @return True if this type's name is equal to type, or if its parent matches
     *         (recursive)
     */
    public boolean matches(String type, boolean includeAll) { // FIXME: The way includesAll is handled does not make
                                                              // sense anymore
        if (type.contains("/")) {
            return matches(type.split("/")[1], includeAll);
        }
        return (includeAll && "all".equals(type)) || name.equals(type)
                || (parent != null && parent.matches(type, includeAll));
    }
